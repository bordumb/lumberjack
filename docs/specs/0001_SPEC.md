...
```
lj init                          # scaffold .lumberjack/, config, gate for this language
lj plan  "<goal>"                # Scout + Foreman only; prints the task graph
lj run   "<goal>" [-n 6]         # plan + execute, rendered live off the ledger
lj status [--json]               # workstreams, leases, conflicts, train position
lj watch                         # Textual TUI dashboard
lj conflicts [--explain <id>] [--json]   # oracle report with hunks and transcript
lj board [--topic ...] [--json]  # read the blackboard
lj land <workstream>             # manual train entry
lj promote                       # integration -> main (human gate)
lj replay <stand-id> [--json]    # rebuild any projection from the ledger
lj halt [--preserve]             # drain and stop
```

Exit codes are part of the interface, because CI reads them: `0` success, `1` partial
(a task blocked, a promote conflicted), `2` usage error, `3` no such stand.  `lj plan`
never executed anything, so its `--dry-run` flag -- which defaulted to true and changed
nothing but a closing hint -- is gone.
